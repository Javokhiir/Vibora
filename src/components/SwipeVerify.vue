<template>
  <div class="relative w-full h-12 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden shadow-lg">
    <!-- Green blur effect on complete -->
    <div v-if="verified" class="absolute inset-0 bg-green-500 blur-xl opacity-60 transition-all duration-300 z-0"></div>

    <!-- Blur trail -->
    <div v-if="dragging" :style="{ left: `0px`, width: `${handleLeft + handleWidth / 2}px` }"
         class="absolute h-full bg-blue-500/70 blur-md z-0 transition-all"></div>

    <!-- Swipe handle -->
    <div ref="handle"
         class="absolute top-0 z-10 w-12 h-full flex items-center justify-center cursor-pointer rounded-full"
    ...
    <div class="absolute inset-0 flex items-center justify-center text-sm font-medium text-gray-700 dark:text-white pointer-events-none z-0">
      <span v-if="!verified">Slayd qilib tasdiqlang</span>
